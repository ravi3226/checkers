...
import { GameBoard, NewlyRegisterGame, PlayerAndBotPair, UserGameResult } from "../config/game.config.js";
import { BotGameTurn, RedisClientResult, RedisStoredValue } from "../config/redis.config.js";
...
export const createGameWithBot = async (io: Server, socket: Socket, payload: any) : Promise<void> => {

    /**
     * validate is this new game creation happening by authorized user or not.
     */
    try {
        const isValidToken : TokenStatus = await validateAuthToken(payload.token)
        ...
        if ( !isValidToken.validate ) {
             ...
             */
            try {
                const lastGame: UserGameResult = await findUserGame(new mongoose.Types.ObjectId(isValidToken.id));

                const isValidGame = lastGame.found ? new Date(lastGame.game[0]?.game[0]?.expiresAt) > new Date() ? true : false : false;

                if ( isValidGame ) {
                    try {
                        const existingGame : RedisClientResult = await redisGetKeyValue(lastGame.game['_id'].toString(), true)
                    ...
                    try {
                         ...
                         */
                        const playersPair: PlayerAndBotPair = await makePlayerPairWithBot(new mongoose.Types.ObjectId(isValidToken.id))
                        ...
                        if (playersPair.success) {
                             ...
                             */
                            try {
                                const newGame : NewlyRegisterGame = await registerGameWithBot(
                                    new mongoose.Types.ObjectId(playersPair.realPlayer['_id']),
                                    new mongoose.Types.ObjectId(playersPair.botPlayer['_id'])
                                );
                                ...
                                if (newGame.success) {
                                     ...
                                     */
                                    try {
                                        const gameOnRedis : RedisStoredValue = await redisSetKeyValue(newGame.game['id'], JSON.stringify({
                                        ...
                                        if (gameOnRedis.success) {
                                            ...
                                            const playerTurn : RedisStoredValue = await redisSetKeyValue(`${BotGameTurn}-${newGame.game['id']}`, 1);
...
export const findUserGame = async (userId: mongoose.Types.ObjectId, numberOfGame: number = 1, decOrAsc: boolean = false) : Promise<UserGameResult> => {
...
export const makePlayerPairWithBot = async (userId: mongoose.Types.ObjectId) : Promise<PlayerAndBotPair> => {
...
export const registerGameWithBot = async (realPlayerId: mongoose.Types.ObjectId, botPlayerId: mongoose.Types.ObjectId) : Promise<NewlyRegisterGame> => {
    return new Promise(async (resolve, reject) => {
        try {
